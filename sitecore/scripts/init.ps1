# Writes hosts-file entries, installs the mkcert root CA, and SitecoreDockerTools itself refuses to
# load without elevation - so fail up front rather than halfway through.
...
# Set the root of the sitecore folder
...
Push-Location $SitecoreRoot\docker\traefik\certs
try {
    $mkcert = ".\mkcert.exe"
...
# .env is gitignored in this repo; start it from the committed template.
if (-not (Test-Path $envFileLocation)) {
    Write-Host "Creating sitecore/.env from .env.example..." -ForegroundColor Green
    Copy-Item "$SitecoreRoot/.env.example" $envFileLocation
}
