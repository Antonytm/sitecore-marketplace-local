...
# Set the root of the sitecore folder: templates -> cm -> build -> docker -> sitecore
$SitecoreRoot = Resolve-Path "$PSScriptRoot\..\..\..\.."
Write-Host @SitecoreRoot

Copy-Item -Path (Join-Path $PSScriptRoot "modules_template") -Destination $modulesPath -Recurse -Force
Copy-Item -Path "$SitecoreRoot\.sitecore"  -Destination $PSScriptRoot -Recurse -Force
...
ReplaceExistingModules -srcConfig "$SitecoreRoot\sitecore.json" -dstConfig $localConfig
