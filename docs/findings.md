...
Authentication turns out to be easier than expected. The foundation-head containers federate
...
**Ongoing:** the SDK is pre-1.0. `AllowedOrigins`, the `:query` suffix convention and the
`host.request` payload are unversioned internals. Versions are pinned exactly, and `pnpm test`
...
| Claim | Status |
|---|---|
| Core SDK implements a usable host mode | ✅ verified — `pnpm test` drives it |
| `origin` bypasses `AllowedOrigins` | ✅ verified — both directions tested |
| Query/mutation suffix convention | ✅ verified |
